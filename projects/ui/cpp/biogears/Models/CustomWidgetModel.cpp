#include "CustomWidgetModel.h"

#include "QVariant"
#include "biogears/BioGearsData.h"

namespace bio {
CustomWidgetModel::CustomWidgetModel(QString prefix, QString name, QAbstractItemModel* parent)
...
//------------------------------------------------------------------------------------
void CustomWidgetModel::append(QString prefix, QString name, const biogears::SEScalar* data)
{
...
//------------------------------------------------------------------------------------
void CustomWidgetModel::append(QString prefix, QString name, const biogears::SEUnitScalar* data)
{
  PhysiologyRequest request;
  request.prefix = prefix;
  request.name = name;
...
//------------------------------------------------------------------------------------
void CustomWidgetModel::modify(int row, const biogears::SEUnitScalar* data)
{
...
//------------------------------------------------------------------------------------
void CustomWidgetModel::modify(int row, const biogears::SEScalar* data)
{
  if (0 <= row && row < _children.size()) {
...
//------------------------------------------------------------------------------------
int CustomWidgetModel::rowCount(const QModelIndex& index) const
{
...
//------------------------------------------------------------------------------------
int CustomWidgetModel::columnCount(const QModelIndex& index) const
{
  if (index.isValid()) {
...
//------------------------------------------------------------------------------------
QVariant CustomWidgetModel::data(const QModelIndex& index, int role) const
{
  if (!index.isValid()) {
    return QVariant();
  }
  ...
  auto request = dynamic_cast<CustomWidgetModel*>(ip);
...
//------------------------------------------------------------------------------------
Qt::ItemFlags CustomWidgetModel::flags(const QModelIndex& index) const
{
  if (!index.isValid()) {
...
//------------------------------------------------------------------------------------
QVariant CustomWidgetModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
//------------------------------------------------------------------------------------
QModelIndex CustomWidgetModel::index(int row, int column, const QModelIndex& parent) const
{
...
//------------------------------------------------------------------------------------
QModelIndex CustomWidgetModel::parent(const QModelIndex& index) const
{
  if (index.internalPointer() == this) {
