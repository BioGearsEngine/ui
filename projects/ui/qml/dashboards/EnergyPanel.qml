...
Item {
	...
	Timer {
		...
		onTriggered : {
			...
			let fatigueIndex = physiologyEnergyModel.index(7, 0) 
			root.fatigue = physiologyEnergyModel.data(fatigueIndex, PhysiologyModel.ValueRole).toFixed(2)
			let o2ConsumptionIndex = physiologyEnergyModel.index(8, 0) 
			root.currentVO2 = physiologyEnergyModel.data(o2ConsumptionIndex, PhysiologyModel.ValueRole).toFixed(2)
			let dehydrationIndex = physiologyEnergyModel.index(9, 0) 
			root.dehydrationFraction = physiologyEnergyModel.data(dehydrationIndex, PhysiologyModel.ValueRole).toFixed(2)
			let ambTempIndex = physiologyEnergyModel.index(10, 0) 
			root.envTemp = physiologyEnergyModel.data(ambTempIndex, PhysiologyModel.ValueRole).toFixed(2)
			let relHumidityIndex = physiologyEnergyModel.index(11, 0) 
