...
UIActionForm {
  ...
  activateData : builderMode ? {"name" : "Apnea", "time" : actionStartTime_s, "severity" : severity} : ({})
  deactivateData : builderMode ? {"name" : "Apnea", "time" : actionStartTime_s + actionDuration_s, "severity" : 0} : ({})
  //Interactive mode -- apply action immediately while running
  onActivate:   { scenario.create_apnea_action(severity)  }
  onDeactivate: { scenario.create_apnea_action(0)  }
